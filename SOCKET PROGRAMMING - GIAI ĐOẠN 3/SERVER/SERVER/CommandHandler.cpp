...
CommandHandler::CommandHandler(const string& clientIp, SOCKET controlSocket)
    : clientIp(clientIp), controlSocket(controlSocket) {}

void CommandHandler::sendIntermediate(const string& msg) {
    // Gửi phản hồi trung gian (VD "150 ...") NGAY qua control socket,
    // trước khi hàm gọi tiếp tục block ở bước transfer UDP.
...
string CommandHandler::handleStor(Session& s, const string& arg) {
    ...
    // Gửi "150" THẬT qua control socket TRƯỚC KHI block chờ nhận UDP,
    // để client kịp mở DataChannel và bắt đầu gửi dữ liệu.
