    ...
    while (true) {
        ...
        if (byteRecv > 0) {
            ...
            if (reply.substr(0, 3) == "150") {
                ...
                if (cmdWord == "STOR" || cmdWord == "APPE") {
                    //STOR: tải file lên, ghi đè | APPE: tải file lên, nối vào cuối file đã có trên server
                    //Cả 2 đều cần client đọc file cục bộ và gửi qua UDP giống nhau
                    DataChannel dc(0);
                    if (dc.start()) {
                ...
                else if (cmdWord == "STOU") {
                    //STOU: server tự đặt tên file từ xa, nhưng client vẫn cần biết gửi file cục bộ nào
                    //=> gõ "STOU <local_file>" giống STOR, filename ở đây chỉ dùng để đọc file cục bộ
                    DataChannel dc(0);
                    if (dc.start()) {
                        dc.sendFile(filename, this->serverIp, SERVER_DATA_PORT);
                        dc.stop();
