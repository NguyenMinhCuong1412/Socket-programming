...
private:
    ...
    //Client báo địa chỉ IP:port của mình (chế độ ACTIVE)
    string handlePort(Session&, const string&);

    //Server tự chọn 1 port, mở sẵn, chờ client kết nối tới (chế độ PASSIVE)
    string handlePasv(Session&);

    //Hủy transfer đang chạy
    string handleAbor(Session&);

    //Chọn port server LẮNG NGHE cho STOR/APPE/STOU dựa theo Session::dataMode
